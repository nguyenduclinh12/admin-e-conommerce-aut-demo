...
const ProductSIZE = () => {
  ...
  const [productSIZEList, setProductSIZEList] = useState(null);
  ...
  const deleteProductSIZE = async (id) => {
    try {
      ...
      const result = await deleteData(`/api/productSIZE`, id);
      if (result?.status === 200) {
        const listRams = await fetchDataFromApi(`/api/productSIZE`);

        if (listRams?.length !== 0) {
          setProductSIZEList(listRams);
        } else {
          throw new Error("Error fetching ProductSIZE data");
        }
        context.setAlertBox({
          open: true,
      ...
      throw new Error(
        result?.data.message || "Error during Delete ProductSIZE "
      );
    } catch (err) {
      context.setAlertBox({
        open: true,
    ...
    try {
      ...
      if (isUpdate) {
        ...
        resultUpload = await editData(
          `/api/productSIZE/${formFields.id}`,
          formFields
        );
        setIsUpdate(false);
      } else {
        resultUpload = await postData("/api/productSIZE/create", formFields);
      }
    ...
    <>
      <div className="right-content w-100">
        <div className="card shadow border-0 w-100 flex-row p-4">
          <h5 className="mb-0">Product SIZE Upload</h5>
          <Breadcrumbs
        ...
        {Array.isArray(productSIZEList) && productSIZEList?.length !== 0 ? (
          <div className="row">
            <div className="col-sm-9">
                          ...
                          <td>
                            <div className="actions d-flex align-items-center">
                              ...
                              <Button
                                className="error"
                                color="error"
                                onClick={() => deleteProductSIZE(item.id)}
                              >
                                <MdDelete></MdDelete>
                              </Button>
                            </div>
